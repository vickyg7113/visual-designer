...
<body>
  ...
  <\/script>
    ...
    `;for(let i=0;i<6;i++){const n=document.createElement("div");n.style.cssText=`
        ...
        display: block;
        flex-shrink: 0;
      `,t.appendChild(n)}e.appendChild(t),this.dragHandle.appendChild(e),e.addEventListener("mousedown",this.handleMouseDown,!0),document.addEventListener("mousemove",this.handleMouseMove,!0),document.addEventListener("mouseup",this.handleMouseUp,!0),window.addEventListener("mousemove",this.handleMouseMove,!0),window.addEventListener("mouseup",this.handleMouseUp,!0)}updateDragHandlePosition(){if(!this.iframe||!this.dragHandle)return;const e=this.iframe.getBoundingClientRect();this.dragHandle.style.top=`${e.top}px`,this.dragHandle.style.left=`${e.left}px`,this.dragHandle.style.width=`${e.width}px`}}const D="visual-designer-guides",y="1.0.0";class A{constructor(e=D){this.storageKey=e}getGuides(){try{const e=localStorage.getItem(this.storageKey);if(!e)return[];const t=JSON.parse(e);return t.version!==y?(console.warn("Storage version mismatch, clearing old data"),this.clear(),[]):t.guides||[]}catch(e){return console.error("Error reading guides from storage:",e),[]}}getGuidesByPage(e){return this.getGuides().filter(i=>i.page===e&&i.status==="active")}saveGuide(e){try{const t=this.getGuides(),i=t.findIndex(o=>o.id===e.id),n={...e,updatedAt:new Date().toISOString(),createdAt:e.createdAt||new Date().toISOString()};i>=0?t[i]=n:t.push(n),this.saveGuides(t)}catch(t){throw console.error("Error saving guide:",t),t}}deleteGuide(e){try{const i=this.getGuides().filter(n=>n.id!==e);this.saveGuides(i)}catch(t){throw console.error("Error deleting guide:",t),t}}saveGuides(e){const t={guides:e,version:y};try{localStorage.setItem(this.storageKey,JSON.stringify(t))}catch(i){throw console.error("Error writing to storage:",i),i}}clear(){try{localStorage.removeItem(this.storageKey)}catch(e){console.error("Error clearing storage:",e)}}getGuide(e){return this.getGuides().find(i=>i.id===e)||null}}class b{constructor(e={}){this.isInitialized=!1,this.isEditorMode=!1,this.exitEditorButton=null,this.redBorderOverlay=null,this.studioBadge=null,this.loadingOverlay=null,this.config=e,this.storage=new A(e.storageKey),this.editorMode=new O,this.guideRenderer=new I,this.editorFrame=new T}init(){if(this.isInitialized){console.warn("SDK already initialized");return}this.isInitialized=!0,this.shouldEnableEditorMode()?(this.showLoadingOverlay(),this.enableEditor()):this.loadGuides(),this.setupEventListeners()}enableEditor(){if(this.isEditorMode)return;this.isEditorMode=!0;let e=typeof window<"u"?window.__visualDesignerMode:null;e||(e=localStorage.getItem("designerModeType")||null),this.editorFrame.create(t=>this.handleEditorMessage(t),e),this.editorMode.activate(t=>this.handleEditorMessage(t)),this.createExitEditorButton(),this.createRedBorderOverlay(),this.createStudioBadge(),localStorage.setItem("designerMode","true"),e&&localStorage.setItem("designerModeType",e),e==="tag-feature"?setTimeout(()=>{this.editorFrame.show(),this.hideLoadingOverlay()},100):setTimeout(()=>{this.hideLoadingOverlay()},300)}disableEditor(){this.isEditorMode&&(this.isEditorMode=!1,this.editorMode.deactivate(),this.editorFrame.destroy(),this.removeExitEditorButton(),this.removeRedBorderOverlay(),this.removeStudioBadge(),localStorage.removeItem("designerMode"),localStorage.removeItem("designerModeType"),this.hideLoadingOverlay(),this.loadGuides())}getGuides(){return this.storage.getGuides()}getGuidesForCurrentPage(){const e=m();return this.storage.getGuidesByPage(e)}saveGuide(e){const t={...e,id:B(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};return this.storage.saveGuide(t),this.isEditorMode||this.loadGuides(),this.config.onGuideSaved&&this.config.onGuideSaved(t),t}deleteGuide(e){this.storage.deleteGuide(e),this.guideRenderer.dismissGuide(e)}loadGuides(){const e=this.storage.getGuides();this.guideRenderer.renderGuides(e)}shouldEnableEditorMode(){return this.config.editorMode!==void 0?this.config.editorMode:!!(typeof window<"u"&&window.__visualDesignerWasLaunched||localStorage.getItem("designerMode")==="true")}handleEditorMessage(e){switch(e.type){case"ELEMENT_SELECTED":this.handleElementSelected(e);break;case"SAVE_GUIDE":this.handleSaveGuide(e);break;case"CANCEL":this.handleCancel();break;case"EXIT_EDITOR_MODE":this.handleExitEditorMode();break;case"EDITOR_READY":this.hideLoadingOverlay();break;default:console.warn("Unknown message type:",e)}}handleElementSelected(e){this.editorFrame.sendElementSelected(e)}handleSaveGuide(e){const t=this.saveGuide({...e.guide,page:m()});console.log("Guide saved:",t)}handleCancel(){this.editorFrame.hide()}handleExitEditorMode(){this.disableEditor()}setupEventListeners(){let e,t;const i=()=>{const n=this.storage.getGuides();this.guideRenderer.updatePositions(n)};window.addEventListener("resize",()=>{clearTimeout(e),e=window.setTimeout(i,100)}),window.addEventListener("scroll",()=>{clearTimeout(t),t=window.setTimeout(i,50)},!0)}isEditorModeActive(){return this.isEditorMode}createExitEditorButton(){if(!this.exitEditorButton){if(!document.body){if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",()=>this.createExitEditorButton());return}setTimeout(()=>this.createExitEditorButton(),100);return}this.exitEditorButton=document.createElement("button"),this.exitEditorButton.id="designer-exit-editor-btn",this.exitEditorButton.textContent="Exit Editor",this.exitEditorButton.style.cssText=`
      position: fixed;
      top: 20px;
      right: 20px;
      padding: 10px 20px;
